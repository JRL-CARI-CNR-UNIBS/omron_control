...
//        controller_interface::interface_configuration_type::ALL};
  ...
  OmronController::on_activate(const rclcpp_lifecycle::State& /**/)
  {
    ...
    if(m_reference_interface_names.size() != 6)
    {
      RCLCPP_ERROR(this->get_node()->get_logger(), "Expected 6 reference interfaces.");
      return controller_interface::CallbackReturn::ERROR;
    }
  ...
  OmronController::update_and_write_commands(const rclcpp::Time&, const rclcpp::Duration &)
  {
    ...
    pose_msg.pose.position.x = state_interfaces_.at(3).get_value();
    ...
    // Command
    if(std::isnan(reference_interfaces_.at(0)) || std::isnan(reference_interfaces_.at(5)))
    {
    ...
    // NOTE: Si aspetta sempre un twist come reference
    command_interfaces_.at(0).set_value(reference_interfaces_.at(0));
    command_interfaces_.at(1).set_value(reference_interfaces_.at(5));
  ...
  OmronController::update_reference_from_subscribers()
  {
    ...
    if (!(!cmd_vel || !(*cmd_vel)))
    {
      if(!std::isnan((*cmd_vel)->twist.linear.x) && !std::isnan((*cmd_vel)->twist.angular.z))
      {
        ...
        reference_interfaces_.at(5) = (*cmd_vel)->twist.angular.z;
      }
      else
      {
