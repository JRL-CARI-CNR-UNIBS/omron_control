...
void OmronLaserClient::pose__cb(ArNetPacket *packet)
{
  ...
  std::lock_guard<std::mutex> lock(m_tf_mutex);
...
//  Eigen::AngleAxisd aax((double) packet->bufToByte2(), Eigen::Vector3d::UnitZ()); // float?
  ...
  m_T_base_map = m_T_map_base.inverse();
}
...
void OmronLaserClient::laser__cb(ArNetPacket *packet)
{
  ...
  Eigen::Affine2d T_map_scan, T_base_scan;
  {
    ...
    {
      T_map_scan.translation().x() = ((double)packet->bufToByte4()) / 1000.0;
      T_map_scan.translation().y() = ((double)packet->bufToByte4()) / 1000.0;
  
      float *pstep = (float*)&actual_scan.data[i * actual_scan.point_step];
      
      T_base_scan = m_T_base_map * T_map_scan;
  
  
      pstep[0] = T_base_scan.translation().x();
      pstep[1] = T_base_scan.translation().y();
